...
void {{class_name}}::packAll(
   ...
   {{kernels['packAll'] | generate_call(cell_interval='ci') | indent(3) }}
}
...
void {{class_name}}::unpackAll(
   ...
   {{kernels['unpackAll'] | generate_call(cell_interval='ci') | indent(3) }}
}
...
void {{class_name}}::localCopyAll(
   ...
   {{kernels['localCopyAll']
     | generate_call(cell_interval={src_field : 'srcInterval', dst_field : 'dstInterval'})
     | indent(3) }}
}
...
void {{class_name}}::packDirection(
   ...
   {{kernels['packDirection'] | generate_call(cell_interval='ci') | indent(3) }}
}
...
void {{class_name}}::unpackDirection(
   ...
   {{kernels['unpackDirection'] | generate_call(cell_interval='ci') | indent(3) }}
}
