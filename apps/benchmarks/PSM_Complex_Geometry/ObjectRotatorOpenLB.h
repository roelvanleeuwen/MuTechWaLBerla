...
typedef field::GhostLayerField< fracSize, 0 > GeometryField_T;
...
class GeometryFieldHandling : public field::BlockDataHandling< GeometryField_T >
{
 ...
 private:
   ...
   GeometryField_T* allocateDispatch(IBlock* const block)
   {
      ...
      return new GeometryField_T(blocks->getNumberOfXCells(*block), blocks->getNumberOfYCells(*block),
...
class ObjectRotatorOpenLB
{
 ...
 public:
   ObjectRotatorOpenLB(shared_ptr< StructuredBlockForest >& blocks, shared_ptr< mesh::TriangleMesh >& mesh, const BlockDataID objectVelocityId,
   ...
   Vector3< uint_t > calculateSize (const std::shared_ptr<StructuredBlockStorage>& blocks_gen, IBlock* const block_gen) {
      Vector3< uint_t > cells(blocks_gen->getNumberOfXCells((*(block_gen))), blocks_gen->getNumberOfYCells((*(block_gen))), blocks_gen->getNumberOfZCells((*(block_gen))));
          return cells;
   }

   void buildGeometryMesh() {
      auto aabbMesh = computeAABB(*mesh_);
      //create field with size of aabb with resolution of refined dx
      auto fieldSize =  Vector3<uint_t>(aabbMesh.xSize() / geometryMeshDx_, aabbMesh.ySize() / geometryMeshDx_, aabbMesh.zSize() / geometryMeshDx_);

      std::shared_ptr< GeometryFieldHandling< GeometryField_T > > geometryFieldDataHandling = std::make_shared< GeometryFieldHandling< GeometryField_T > >(blocks_, uint_c(2));

      geometryField_ = field::addToStorage< GeometryField_T >(blocks_,  "geometryField_" + meshName_, fieldSize, fracSize(0.0), field::fzyx, 1);
      blocks_->addBlockData( const shared_ptr< T > & dataHandling,
 ...
 private:
   ...
   Field<fracSize ,1> geometryField_;
   ...
   real_t geometryMeshDx_;

   shared_ptr<GeometryOctreeNode> geometryOctree_;
   };


void fuseFractionFields(shared_ptr< StructuredBlockForest >& blocks, BlockDataID dstFracFieldID, std::vector<shared_ptr<ObjectRotatorOpenLB>> objectRotators, uint_t timestep, uint_t rotationFrequency) {
