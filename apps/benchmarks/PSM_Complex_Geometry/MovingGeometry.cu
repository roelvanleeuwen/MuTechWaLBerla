...
void MovingGeometry::getFractionFieldFromGeometryMesh(uint_t timestep)  {
   ...
   for (auto& block : *blocks_) {
      ...
      auto fractionFieldGPU = block.getData< gpu::GPUField<real_t> >(fractionFieldId_);
      real_t * RESTRICT const _data_fractionFieldGPU = fractionFieldGPU->dataAt(0, 0, 0, 0);
      WALBERLA_LOG_INFO_ON_ROOT("Get _data_geometryFieldGPU")
      geoSize * RESTRICT const _data_geometryFieldGPU = geometryFieldsGPU_[level]->dataAt(0, 0, 0, 0);
      WALBERLA_LOG_INFO_ON_ROOT("Finished Get _data_geometryFieldGPU")
      ...
      int3 geometry_field_size = {int(geometryFieldsGPU_[level]->xSize()), int(geometryFieldsGPU_[level]->ySize()), int(geometryFieldsGPU_[level]->zSize()) };
      int3 geometry_field_stride = {int(geometryFieldsGPU_[level]->xStride()), int(geometryFieldsGPU_[level]->yStride()), int(geometryFieldsGPU_[level]->zStride())};
