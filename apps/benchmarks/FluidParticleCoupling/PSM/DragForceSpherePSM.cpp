...
class DragForceEvaluator
{
 public:
   DragForceEvaluator(SweepTimeloop* timeloop, Setup* setup, const shared_ptr< StructuredBlockStorage >& blocks,
      ...
      : timeloop_(timeloop), setup_(setup), blocks_(blocks), flagFieldID_(flagFieldID), pdfFieldID_(pdfFieldID),
        ac_(ac), sphereID_(sphereID), normalizedDragOld_(0.0), normalizedDragNew_(0.0)
   {
...
int main(int argc, char** argv)
{
   ...
   for (int i = 1; i < argc; ++i)
   {
      ...
      if (std::strcmp(argv[i], "--MEMVariant") == 0)
      {
         method = to_MEMVariant(argv[++i]);
         continue;
      }
...
#   endif
   ...
   // optimizedPDFCommunicationScheme.addPackInfo( make_shared< field::communication::PackInfo< PdfField_T > >(
   // pdfFieldID ) );
   ...
   if (method == MEMVariant::CLI)
   {
      ...
      ps->forEachParticle(false, mesa_pd::kernel::SelectAll(), *accessor, movingParticleMappingKernel, *accessor,
                          MO_CLI_Flag);
   }
   else
   {
   ...
   if (useOmegaBulkAdaption)
   {
      using OmegaBulkAdapter_T =
         lbm_mesapd_coupling::OmegaBulkAdapter< ParticleAccessor_T, mesa_pd::kernel::SelectAll >;
      ...
      shared_ptr< OmegaBulkAdapter_T > omegaBulkAdapter =
         make_shared< OmegaBulkAdapter_T >(blocks, omegaBulkFieldID, accessor, defaultOmegaBulk, omegaBulk,
                                           adaptionLayerSize, mesa_pd::kernel::SelectAll());
   ...
   auto forceEval =
      make_shared< DragForceEval_T >(&timeloop, &setup, blocks, flagFieldID, pdfFieldID, accessor, sphereID);

#ifdef WALBERLA_BUILD_WITH_CODEGEN

   auto lbmSweep = LatticeModel_T::Sweep(pdfFieldID);
   // splitting stream and collide is currently buggy in lbmpy with vectorization -> build without optimize for local
   // host and vectorization collide LBM step
