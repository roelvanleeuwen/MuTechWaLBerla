...
//
//! \file   ArmadilloMeshDecomposition.cpp
//! \author Tobias Leemann <tobias.leemann@fau.de>
//! \brief Showcase for the use of convex decomposition in a complex scenario.
//! This includes exact decomposition, approximate decomposition and the reuse of decomposition results.
//
//======================================================================================================================
...
/* ---------------------------------------------------------------------------------------------------------------------
 * HELPER FUNCTION TO WRITE MESHES FOR COMPLEX BODIES.
 */
...
/* Fill a cube to cut out of parts of the hopper below z = 0 */
...
/* Write the hopper and the prism-assembly mesh */
void writeConcaveBodies(){
   ...
   WALBERLA_LOG_INFO_ON_ROOT("Writing prism assembly");
   ...
   std::ofstream output;
   output.open("PrismAssembly.off");
   output << outMesh;
   output.close();
   WALBERLA_LOG_INFO_ON_ROOT("Wrote prism assembly.");

   WALBERLA_LOG_INFO_ON_ROOT("Writing hopper");

   // Parameters for the hopper
   const double d_inlet = 44.0; // Inner Diameter of the inlet
   const double d_outlet = 23.0; // Inner Diameter of the outlet
   const double side_width = 2.0; // Width of the hopper boundary,
   // outer diameter = inner diameter + 2*side_width
   ...
   //Write four sides and turn them accordingly using CGAL.
   Polyhedron polyhop;
   ...
   //Cut of lower part of pyramide below outlet
   Polyhedron pCube;
   ...
   output << outMeshHop;
   output.close();
   WALBERLA_LOG_INFO_ON_ROOT("Wrote hopper.");
}


/* ---------------------------------------------------------------------------------------------------------------------
 * END HELPER FUNCTIONS. MAIN CODE STARTS HERE.
 * ---------------------------------------------------------------------------------------------------------------------
 */
int main( int argc, char ** argv ) {
   // Use with -write to only write the required bodies (the hopper and the other non-convex prism-assembly)
   ...
   if (!input.is_open() || !inputHopper.is_open()){
      // Try writing the bodies if one of them was not found.
      input.close();
      inputHopper.close();
      WALBERLA_LOG_INFO_ON_ROOT("*** Bodies not found. Writing Bodies... ***");
      writeConcaveBodies();
      input.open("PrismAssembly.off");
      inputHopper.open("Hopper.off");
   }
   ...
   if (!input.is_open() || !inputHopper.is_open()){
      // Still not found -> other error. Exist
      WALBERLA_LOG_INFO_ON_ROOT("*** Bodies still not found. Exiting. ***");
      return EXIT_FAILURE;
   }

   inputArmadillo.open("Armadillo.off");
   if (!inputArmadillo.is_open() ){
      // Still not found -> other error. Exist
      WALBERLA_LOG_INFO_ON_ROOT("*** Armadillo.off not found. Exiting. ***");
      return EXIT_FAILURE;
   }

   WALBERLA_LOG_INFO_ON_ROOT("*** Reading meshes ***");
   mesh::TriangleMesh PrismMesh;
   mesh::TriangleMesh hopperMesh;
   mesh::TriangleMesh armadilloMesh;

   // OpenMesh read options
   OpenMesh::IO::Options opt(OpenMesh::IO::Options::Default);

   OpenMesh::IO::ImporterT<mesh::TriangleMesh> importer(PrismMesh);
   ...
   // Scale Armadillo to fit through the hopper
   auto factor = real_t(0.15);
   mesh::scale(armadilloMesh, Vec3(factor,factor,factor));
   ...
   /* Create union of the convex parts of the Armadillo using approximate convex decomposition
    */
   mesh::pe::TriangleMeshUnion* unArmadillo = mesh::pe::createApproximateNonConvexUnion( *globalBodyStorage, *forest, storageID, 0, Vec3(0,0,50), armadilloMesh);

   Vec3 rndVel(math::realRandom<real_t>(-vMax, vMax), math::realRandom<real_t>(-vMax, vMax), math::realRandom<real_t>(-vMax, vMax));
   if (unArmadillo != nullptr) unArmadillo->setLinearVel(rndVel);

   /* Create union of the convex parts of the Hopper using exact convex decomposition
    */
   ...
   /* Explicit exact decomposition of the Prism assmbly.
    * Use this call to obtain a vector of the convex parts, which can be used to create multiple particles
    * of the same geometry without redoing the decomposition. (See particle creation in this example)
    * Calls to createNonConvexUnion or createApproximateNonConvexUnion will perform the decomposition each time called.
    */
   std::vector<mesh::TriangleMesh> prismParts = mesh::ConvexDecomposer::convexDecompose(PrismMesh);


   // Create Boxes that support the hopper
   createBox( *globalBodyStorage, *forest, storageID, 0, Vec3(-20,0,real_t(11.5)), Vec3(9,40,23), Material::find("iron"), false, true, false);
   createBox( *globalBodyStorage, *forest, storageID, 0, Vec3(20,0,real_t(11.5)), Vec3(9,40,23), Material::find("iron"), false, true, false);


   WALBERLA_LOG_INFO_ON_ROOT("*** CREATING PARTICLES ***");
   ...
   for (auto blkIt = forest->begin(); blkIt != forest->end(); ++blkIt)
   {
      ...
      {
         /* Create union of the convex parts of the Hopper by reusing the results of the decomposition */
         mesh::pe::TriangleMeshUnion* particle =  mesh::pe::createNonConvexUnionOfConvexParts( *globalBodyStorage, *forest, storageID, numParticles, *it, prismParts);
