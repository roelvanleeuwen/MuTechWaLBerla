...
using Communication_T = blockforest::SimpleCommunication< CommunicationStencilFluid_T >;
...
int main(int argc, char** argv)
{
   ...
   const real_t liquidDepth = domainSize[1] / 2;
   ...
   LatticeModelFluid_T latticeModel = LatticeModelFluid_T(forceDensityFieldID, relaxationRate);
   ...
   PdfCommunicationFluid_T pdfCommunication(blockForest, pdfFieldID);
   pdfCommunication();
   ...
   initHydrostaticPressure< PdfFieldFluid_T >(blockForest, pdfFieldID, Vector3<real_t>(real_c(0), (*acceleration)[1], real_c(0)), liquidDepth);
   ...
   setDensityInNonFluidCellsToOne< FlagField_T, PdfFieldFluid_T >(blockForest, flagInfo, flagFieldID, pdfFieldID);
