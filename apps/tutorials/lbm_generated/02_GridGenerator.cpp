...
int main(int argc, char** argv)
{
   ...
   const uint_t numProcesses    = domainSetup.getParameter< uint_t >("numProcesses");
   const std::string blockForestFilestem =
      domainSetup.getParameter< std::string >("blockForestFilestem", "blockforest");
   const bool writeVtk         = domainSetup.getParameter< bool >("writeVtk", false);
   const bool outputStatistics = domainSetup.getParameter< bool >("outputStatistics", false);

   SetupBlockForest setupBfs;

   const blockforest::AABBRefinementSelection refinementSelection(domainSetup);

   setupBfs.addRefinementSelectionFunction(std::function< void(SetupBlockForest&) >(refinementSelection));
   ...
   if (writeVtk) { setupBfs.writeVTKOutput(blockForestFilestem); }
