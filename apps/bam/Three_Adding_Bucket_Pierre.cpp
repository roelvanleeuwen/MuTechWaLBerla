...
//////////
    ...
    int main( int argc, char **argv )
    {
        ...
        // simulation control
        std::string baseFolder = "vtk_Adding_Bucket_Pierre";
        ...
        uint_t vtkSpacing = uint_t(500);
        uint_t timeSteps = uint_t(8000);
        ...
        real_t diameter_SI = real_t(75e-5);
        ...
        real_t gravitationalAcceleration_SI = real_t(5); // m / s**2

        //Adding bucket properties:
        real_t bucketWallThickness_SI {0.001_r};
        real_t bucketHeight_SI {0.01_r};
        real_t bucketRadius_SI {0.005_r};
        size_t numBucketWallElements {10};
        Vector3<real_t> bucketVelocity_SI {0_r, 0_r, -0.0025_r};
        Vector3<real_t> bucketBottomCenterPosition_SI {domainSize_SI / 2_r };
        ...
        real_t bucketWallThickness =  bucketWallThickness_SI/dx_SI;
        real_t bucketHeight =  bucketHeight_SI/dx_SI;
        real_t bucketRadius = bucketRadius_SI/dx_SI;
        Vector3<real_t> bucketVelocity(bucketVelocity_SI[0] * dt_SI/dx_SI,bucketVelocity_SI[1] * dt_SI/dx_SI,bucketVelocity_SI[2] * dt_SI/dx_SI);
        Vector3<real_t> bucketBottomCenterPosition(bucketBottomCenterPosition_SI[0]/dx_SI ,bucketBottomCenterPosition_SI[1]/dx_SI,bucketBottomCenterPosition_SI[2]/dx_SI* 0.87_r);
        //*********
//
        ...
        WALBERLA_LOG_INFO_ON_ROOT("box size = " << domainSize);
        ...
        initSpheresFromFile("logging_spheres.txt", *ps, *mesapdDomain, sphereShape);
        ...
        ps->forEachParticle(false, SphereAndBoxSelector(), *accessor, movingParticleMappingKernel, *accessor, MO_Flag);
        ...
        ///Bucket creation
        /// Bucket Creation
        WALBERLA_LOG_INFO_ON_ROOT("Creating bucket at position " << bucketBottomCenterPosition);
        const auto bucketElementUids = createBucket(*ps, *ss,
                                                    bucketRadius, bucketWallThickness, bucketHeight, bucketBottomCenterPosition,
                                                    numBucketWallElements,
                                                    bucketVelocity);
        ...
        if( vtkSpacing != uint_t(0) )
        {
            ...
            boxVtkOutput->addOutput("edgeLengths",
                                    std::make_shared< walberla::mesa_pd::vtk::OutputSelector<SelectBoxEdgeLengths>>(SelectBoxEdgeLengths(*ss)));
            boxVtkOutput->addOutput<SelectRotation>("rotation");
            boxVtkOutput->setParticleSelector([ss](const mesa_pd::data::ParticleStorage::iterator& pIt){
                return ss->shapes[pIt->getShapeID()]->getShapeType() == mesa_pd::data::Box::SHAPE_TYPE;
            });
            auto boxVtkWriter = walberla::vtk::createVTKOutput_PointData(boxVtkOutput, "bucket", vtkSpacing,baseFolder, "simulation_step");
        ...
        timeloopAfterParticle.add() << Sweep( lbm_mesapd_coupling::makeMovingParticleMapping<PdfField_T, BoundaryHandling_T>(blocks, pdfFieldID, boundaryHandlingID, particleFieldID, accessor, MO_Flag, FormerMO_Flag, SphereAndBoxSelector(), false), "Particle Mapping" );
        ...
        for (uint_t i = 0; i < timeSteps; ++i )
        {
            ...
            for(uint_t subCycle = 0; subCycle < numberOfMesapdSubCycles; ++subCycle )
            {
                ...
                updateBucketPosition(*accessor, bucketElementUids, timeStepSizeMesapd);
