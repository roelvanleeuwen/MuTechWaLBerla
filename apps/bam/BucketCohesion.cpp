...
int main(int argc, char** argv) {
   ...
   const uint_t numberOfTimeSteps {40000};
   const uint_t numberOfAdditionalTimeStepsWithBox {0};
   ...
   real_t bucketWallThickness {0.001_r};
   real_t bucketHeight {0.03_r};
   real_t bucketRadius {0.015_r};
   size_t numBucketWallElements {10};
   Vector3<real_t> bucketVelocity {0_r, 0_r, -0.05_r};
   Vector3<real_t> bucketBottomCenterPosition {domainSize_SI / 2_r};
   ...
   real_t effectiveMass_SphereSphere = massSphere_SI * massSphere_SI / (2_r * massSphere_SI);
...
//                                 [&dem,&cohesion, &mesapdDomain, timeStepSize_SI]
...
//
////                                         cohesion(acd.getIdx1(), acd.getIdx2(), ac,
////                                                  true, acd.getContactNormal(), acd.getPenetrationDepth());
//                                      }
//                                   }
//                                 },
//                                 *accessor );
...
//
   ...
   bucketBottomCenterPosition[2] = maximumParticleHeightInSystem + diameter_SI;

   /// Bucket Creation
   WALBERLA_LOG_INFO_ON_ROOT("Creating bucket at position " << bucketBottomCenterPosition);
   const auto bucketElementUids = createBucket(*particleStorage, *shapeStorage,
                                               bucketRadius, bucketWallThickness, bucketHeight, bucketBottomCenterPosition,
                                               numBucketWallElements,
                                               bucketVelocity);

   /// VTK Output of Bucket
   auto boxVtkOutput = make_shared<vtk::ParticleVtkOutput>(particleStorage);
   boxVtkOutput->addOutput("edgeLengths",
                           std::make_shared<vtk::OutputSelector<SelectBoxEdgeLengths>>(SelectBoxEdgeLengths(*shapeStorage)));
   boxVtkOutput->addOutput<SelectRotation>("rotation");
   boxVtkOutput->setParticleSelector([shapeStorage](const data::ParticleStorage::iterator& pIt){
     return shapeStorage->shapes[pIt->getShapeID()]->getShapeType() == data::Box::SHAPE_TYPE;
   });
   auto boxVtkWriter = walberla::vtk::createVTKOutput_PointData(boxVtkOutput, "bucket", 1,
                                                                vtkOutputFolder, "simulation_step");
   ...
   auto particleVtkWriter = walberla::vtk::createVTKOutput_PointData(particleVtkOutput, "particles", 1,
                                                                     vtkOutputFolder, "simulation_step");
...
//   mesa_pd::kernel::CohesionInitialization cohesionInitialization;
   ...
   // Reset sphere velocities before cohesion initialization
   ...
   uint_t timeStepsWithBox = uint_c( std::ceil( (bucketBottomCenterPosition[2] / -bucketVelocity[2]) / timeStepSize_SI )) + numberOfAdditionalTimeStepsWithBox;
