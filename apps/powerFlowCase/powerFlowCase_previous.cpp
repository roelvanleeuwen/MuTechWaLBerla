...
int main(int argc, char** argv)
{
   ...
   Description: The flow parameters specify the initial state of the flow. For this there is an angle of attack and a
   velocity magnitude. The initial velocity is calculated based on the angle of attack and the velocity magnitude.
   */
   ...
   Description: The simulation parameters are used for the simulation time, the use of the GUI and the terminal
   simulation logger. The GUI will not be used since Qt4 is not supported anymore by linux systems.
   */
   ...
   */
   ...
   const real_t decreasePowerFlowDomainFactor =
   ...
   WALBERLA_ROOT_SECTION() { distanceOctree->writeVTKOutput("vtk_out/distanceOctree"); }
   ...
   const real_t airfoilChordLength   = aabb.xSize();
   const real_t airfoilChordLengthLU = aabb.xSize() / dx;
   ...
   const Vector3< uint_t > cellsPerBlock =
      Vector3< uint_t >(result.cellsPerBlock_x, result.cellsPerBlock_x, 16); // The z direction has 16 cells per block
   ...
   if (scalePowerFlowDomain)
   {
      ...
      domainScaling =
         Vector3< real_t >(100 * decreasePowerFlowDomainFactor * xyAdjuster_x,
                           100 * aabb.xSize() / aabb.ySize() * decreasePowerFlowDomainFactor * xyAdjuster_y,
                           static_cast< real_t >(cellsPerBlock[2]) * dx / aabb.zSize());
   }
   else { domainScaling = domainParameters.getParameter< Vector3< real_t > >("domainScaling", Vector3< real_t >(1)); }
   ...
   real_t nBlocks_x =
      static_cast< real_t >(result.nBlocks_x); // std::round(aabb.xSize() / dx / static_cast<real_t>(cellsPerBlock[0]));
   real_t nBlocks_y =
      static_cast< real_t >(result.nBlocks_x); // std::round(aabb.ySize() / dx / static_cast<real_t>(cellsPerBlock[1]));
   ...
   else
   {
      ...
      WALBERLA_LOG_INFO_ON_ROOT(
         "Total number of unrefined cells: " << cellsPerBlock[0] * static_cast< uint_t >(nBlocks_x) * cellsPerBlock[1] *
                                                   static_cast< uint_t >(nBlocks_y) * cellsPerBlock[2] *
                                                   static_cast< uint_t >(nBlocks_z))
   }
   //! [aabb]
   ...
   bfc.setRefinementSelectionFunction(makeRefinementSelection(distanceOctree, numLevels - 1, dx, dx * real_t(1)));
   ...
   BlockDataID pdfFieldId      = lbm::addPdfFieldToStorage(
           blocks, "pdf field", latticeModel, initialVelocity, real_t(1),
           numGhostLayers); // Here the initialisation of the pdf field. This includes the initial velocity and density
   ...
   real_t speedOfSoundList = std::pow(1.4 * 287.15 * 281, 0.5);                // speed of sound in physical units
   real_t MachList         = flowVelocity / speedOfSoundList;                  // Mach number in physical units
   real_t ReList           = flowVelocity * airfoilChordLength / kinViscosity; // Reynolds number
   ...
   std::vector< real_t > kinViscosityLUList(numLevels);
   std::vector< real_t > omegaList(numLevels);
   std::vector< real_t > flowVelocityLUList(numLevels);
   std::vector< real_t > dxList(numLevels);
   std::vector< real_t > dtList(numLevels);
   std::vector< real_t > speedOfSoundLUList(numLevels);
   std::vector< real_t > MachLUList(numLevels);

   for (uint_t i = 0; i < numLevels; ++i)
   {
      dxList[i]             = dx / std::pow(2, i);
      ...
      dtList[i]             = kinViscosityLUList[i] * std::pow(dxList[i], 2) / kinViscosity; // time step
      flowVelocityLUList[i] = flowVelocity * dtList[i] / dxList[i];
