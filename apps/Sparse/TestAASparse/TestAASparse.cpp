...
int main(int argc, char **argv)
{
   ...
   if (vtkWriteFrequency > 0)
   {
      ...
      auto velWriter = make_shared< lbm::ListVelocityVTKWriter< List_T, float > >(pdfListId, "velocity");
   ...
   WALBERLA_LOG_INFO_ON_ROOT("Simulating TestAASparse:"
                             "\n timesteps:                  "
   ...
   for (int outerIteration = 0; outerIteration < outerIterations; ++outerIteration) {
      ...
      WALBERLA_ROOT_SECTION() {
         std::string outFile = parameters.getParameter< std::string >("outFile", "Pull");
         outFile.append("_").append(std::to_string(nrOfProcesses)).append("_nodes.txt");
         std::ofstream myfile (outFile, std::ios_base::app);
         if (myfile.is_open())
         {
            myfile << nrOfProcesses << " " << performance.mflupsPerProcess( timesteps,  time ) << " " << performance.mlupsPerProcess( timesteps,  time ) << " " <<  InnerOuterSplit << " " << runBoundaries << std::endl ;
            myfile.close();
         }
      }
