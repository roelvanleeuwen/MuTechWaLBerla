...
class BlockDataHandling : public blockforest::BlockDataHandling< Field_T >
{
 public:
   ...
   Field_T* initialize(IBlock* const block) override
   {
      ...
      if (initFunction_) initFunction_(field, block);
   ...
   void serializeCoarseToFine(Block* const block, const BlockDataID& id, mpi::SendBuffer& buffer,
                              const uint_t child) override;
   ...
   void deserializeFineToCoarse(Block* const block, const BlockDataID& id, mpi::RecvBuffer& buffer,
                                const uint_t child) override;
 ...
 protected:
   ...
   struct Merge
   {
      ...
      {
         return Pseudo2D ? static_cast< T >(value / numeric_cast< T >(4)) :
                           static_cast< T >(value / numeric_cast< T >(8));
      }
   };
   ...
   {
      static Vector2< T > result(const Vector2< T >& value)
      {
         return Pseudo2D ? (value / numeric_cast< T >(4)) : (value / numeric_cast< T >(8));
      }
   };
   ...
   {
      static Vector3< T > result(const Vector3< T >& value)
      {
         return Pseudo2D ? (value / numeric_cast< T >(4)) : (value / numeric_cast< T >(8));
      }
   };
...
inline void BlockDataHandling< Field_T, Pseudo2D >::serialize(IBlock* const block, const BlockDataID& id,
...
template< typename Field_T, bool Pseudo2D >
void BlockDataHandling< Field_T, Pseudo2D >::serializeCoarseToFine(Block* const block, const BlockDataID& id,
                                                                   mpi::SendBuffer& buffer, const uint_t child)
{
   Field_T* field = block->template getData< Field_T >(id);
   WALBERLA_ASSERT_NOT_NULLPTR(field);
   ...
   const cell_idx_t zBegin =
      Pseudo2D ? cell_idx_t(0) : ((child & uint_t(4)) ? (cell_idx_c(zSize) / cell_idx_t(2)) : cell_idx_t(0));
   const cell_idx_t zEnd =
      Pseudo2D ? cell_idx_t(1) : ((child & uint_t(4)) ? cell_idx_c(zSize) : (cell_idx_c(zSize) / cell_idx_t(2)));
...
template< typename Field_T, bool Pseudo2D >
void BlockDataHandling< Field_T, Pseudo2D >::serializeFineToCoarse(Block* const block, const BlockDataID& id,
                                                                   mpi::SendBuffer& buffer)
{
   Field_T* field = block->template getData< Field_T >(id);
   WALBERLA_ASSERT_NOT_NULLPTR(field)
...
#ifndef NDEBUG
   buffer << block->getId().getBranchId() << (xSize / uint_t(2)) << (ySize / uint_t(2))
          << (Pseudo2D ? zSize : (zSize / uint_t(2))) << fSize;
#endif
         ...
         for (cell_idx_t x = cell_idx_t(0); x < cell_idx_c(xSize); x += cell_idx_t(2))
         {
            for (uint_t f = uint_t(0); f < fSize; ++f)
            {
               ...
               if (!Pseudo2D)
               {
                  ...
                  result = static_cast< Value_T >(
                     result + field->get(x + cell_idx_t(1), y + cell_idx_t(1), z + cell_idx_t(1), f));
               }
...
template< typename Field_T, bool Pseudo2D >
inline void BlockDataHandling< Field_T, Pseudo2D >::deserialize(IBlock* const block, const BlockDataID& id,
...
template< typename Field_T, bool Pseudo2D >
void BlockDataHandling< Field_T, Pseudo2D >::deserializeCoarseToFine(Block* const block, const BlockDataID& id,
                                                                     mpi::RecvBuffer& buffer)
{
   Field_T* field = block->template getData< Field_T >(id);
   ...
   if (Pseudo2D) { WALBERLA_ASSERT_EQUAL(zSender, zSize) }
   else
   {
...
template< typename Field_T, bool Pseudo2D >
void BlockDataHandling< Field_T, Pseudo2D >::deserializeFineToCoarse(Block* const block, const BlockDataID& id,
                                                                     mpi::RecvBuffer& buffer, const uint_t child)
{
   Field_T* field = block->template getData< Field_T >(id);
...
{
   if constexpr (GhostLayerField_T::OLD)
   {
      return new GhostLayerField_T(x, y, z, gl, initValue, l, alloc);
   }
   else
   {
...
template<>
inline FlagField< uint8_t >* allocate(const uint_t x, const uint_t y, const uint_t z, const uint_t /*fSize*/,
                                      const uint_t gl, const uint8_t&, Layout,
                                      const shared_ptr< field::FieldAllocator< uint8_t > >& alloc)
{
   return new FlagField< uint8_t >(x, y, z, gl, alloc);
}
template<>
inline FlagField< uint16_t >* allocate(const uint_t x, const uint_t y, const uint_t z, const uint_t /*fSize*/,
                                       const uint_t gl, const uint16_t&, Layout,
                                       const shared_ptr< field::FieldAllocator< uint16_t > >& alloc)
{
   return new FlagField< uint16_t >(x, y, z, gl, alloc);
}
template<>
inline FlagField< uint32_t >* allocate(const uint_t x, const uint_t y, const uint_t z, const uint_t /*fSize*/,
                                       const uint_t gl, const uint32_t&, Layout,
                                       const shared_ptr< field::FieldAllocator< uint32_t > >& alloc)
{
   return new FlagField< uint32_t >(x, y, z, gl, alloc);
}
template<>
inline FlagField< uint64_t >* allocate(const uint_t x, const uint_t y, const uint_t z, const uint_t /*fSize*/,
                                       const uint_t gl, const uint64_t&, Layout,
...
template< typename GhostLayerField_T >
inline GhostLayerField_T* allocate(const uint_t x, const uint_t y, const uint_t z, const uint_t fSize, const uint_t gl, Layout l,
                                   const shared_ptr< field::FieldAllocator< typename GhostLayerField_T::value_type > >& alloc = nullptr)
{
   if constexpr (GhostLayerField_T::OLD)
   {
   ...
   {
      return new GhostLayerField_T(x, y, z, fSize, gl, l, alloc);
   }
}
template<>
inline FlagField< uint8_t >* allocate(const uint_t x, const uint_t y, const uint_t z, const uint_t /*fSize*/,
                                       const uint_t gl, Layout,
                                       const shared_ptr< field::FieldAllocator< uint8_t > >& alloc)
{
   return new FlagField< uint8_t >(x, y, z, gl, alloc);
}
...
{
   return Vector3< uint_t >(blocks->getNumberOfXCells(*block), blocks->getNumberOfYCells(*block),
                            blocks->getNumberOfZCells(*block));
}

template<typename FlagField_T>
inline FlagField_T* allocateflagfield(const uint_t x, const uint_t y, const uint_t z, const uint_t /*fSize*/,
                                      const uint_t gl, const uint8_t&, Layout,
                                      const shared_ptr< field::FieldAllocator< uint8_t > >& alloc)
{
   return new FlagField_T(x, y, z, gl, alloc);
}
...
class DefaultBlockDataHandling : public BlockDataHandling< GhostLayerField_T >
{
 public:
   ...
   DefaultBlockDataHandling(
      const weak_ptr< StructuredBlockStorage >& blocks, const uint_t fSize,
      ...
      : blocks_(blocks), fSize_(fSize), nrOfGhostLayers_(uint_t(1)), initValue_(), layout_(zyxf),
        calculateSize_(calculateSize), alloc_(alloc)
   {}

   DefaultBlockDataHandling(
      const weak_ptr< StructuredBlockStorage >& blocks, const uint_t fSize, const uint_t nrOfGhostLayers,
      const std::function< Vector3< uint_t >(const shared_ptr< StructuredBlockStorage >&, IBlock* const) >
         calculateSize                                           = internal::defaultSize,
      const shared_ptr< field::FieldAllocator< Value_T > > alloc = nullptr)
      : blocks_(blocks), fSize_(fSize), nrOfGhostLayers_(nrOfGhostLayers), initValue_(), layout_(zyxf),
        calculateSize_(calculateSize), alloc_(alloc)
   {}
        ...
        calculateSize_(calculateSize), alloc_(alloc)
   {
      static_assert(!std::is_same< GhostLayerField_T, FlagField< Value_T > >::value,
                    "When using class FlagField, only constructors without the explicit specification of an initial "
                    "value and the field layout are available!");
   }
 ...
 protected:
   GhostLayerField_T* allocate(IBlock* const block) override
   {
      ...
      WALBERLA_CHECK_NOT_NULLPTR(
         blocks, "Trying to access 'DefaultBlockDataHandling' for a block storage object that doesn't exist anymore")
      const Vector3< uint_t > size = calculateSize_(blocks, block);
      return internal::allocate< GhostLayerField_T >(size[0], size[1], size[2], fSize_, nrOfGhostLayers_, initValue_,
                                                     layout_, alloc_);
   }
   ...
   GhostLayerField_T* reallocate(IBlock* const block) override
   {
      ...
      WALBERLA_CHECK_NOT_NULLPTR(
         blocks, "Trying to access 'DefaultBlockDataHandling' for a block storage object that doesn't exist anymore")
      const Vector3< uint_t > size = calculateSize_(blocks, block);
      return internal::allocate< GhostLayerField_T >(size[0], size[1], size[2], fSize_, nrOfGhostLayers_, layout_,
                                                     alloc_);
   }
...
class AlwaysInitializeBlockDataHandling : public blockforest::AlwaysInitializeBlockDataHandling< GhostLayerField_T >
{
 public:
   ...
   GhostLayerField_T* initialize(IBlock* const block) override
   {
      ...
      WALBERLA_CHECK_NOT_NULLPTR(
         blocks,
         "Trying to access 'AlwaysInitializeBlockDataHandling' for a block storage object that doesn't exist anymore")
      Vector3< uint_t > size   = calculateSize_(blocks, block);
      GhostLayerField_T* field = internal::allocate< GhostLayerField_T >(size[0], size[1], size[2], fSize_,
...
{
 public:
   CloneBlockDataHandling(const ConstBlockDataID& fieldToClone) : fieldToClone_(fieldToClone) {}
...
class FlattenedShallowCopyBlockDataHandling
   : public blockforest::AlwaysInitializeBlockDataHandling< typename Field_T::FlattenedField >
{
 public:
   FlattenedShallowCopyBlockDataHandling(const ConstBlockDataID& fieldToClone) : fieldToClone_(fieldToClone) {}
