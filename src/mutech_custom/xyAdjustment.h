...
struct AdjustmentResult
{
   real_t xyAdjustment;
   uint_t nBlocks_x;
   uint_t cellsPerBlock_x;
};

AdjustmentResult xyAdjustment(const real_t xSize, const real_t decreasePowerFlowDomainFactor, const real_t dx)
{
   real_t nBlocks_x          = 3;
   real_t nBlocks_x_remember = nBlocks_x;

   real_t cellsPerBlock_x          = xSize * decreasePowerFlowDomainFactor / (dx * nBlocks_x);
   real_t cellsPerBlock_x_remember = cellsPerBlock_x;

   if (cellsPerBlock_x < 16.0)
   {
      std::cout << "The number of cells in the x and z direction are below 16. This is not allowed. The "
                   "decreasePowerFlowDomainFactor should be "
   ...
   real_t xyAdjustment = (nBlocks_x * cellsPerBlock_x * dx) / (xSize * decreasePowerFlowDomainFactor);

   return { xyAdjustment, static_cast< uint_t >(nBlocks_x), static_cast< uint_t >(cellsPerBlock_x) };
}
/*
void testOne ()
{
    // Test case 1
    const real_t xSize = 20;
    const real_t decreasePowerFlowDomainFactor = 0.013;
    const real_t dx = 0.0051015625;
...
void testTwo ()
{
    // Test case 1
    const real_t xSize = 20;
    const real_t decreasePowerFlowDomainFactor = 0.02;
    const real_t dx = 0.0051015625;
    auto result = xyAdjustment(xSize, decreasePowerFlowDomainFactor, dx);
...
void testThree ()
{
    // Test case 1
    const real_t xSize = 20;
    const real_t decreasePowerFlowDomainFactor = 0.5;
    const real_t dx = 0.0051015625;
    auto result = xyAdjustment(xSize, decreasePowerFlowDomainFactor, dx);
