...
//{
//   if (py::isinstance< py::tuple >(obj)) return false;
//
//   py::tuple t = py::tuple(obj);
//   return py::len(t) == 3;
//}
//
//class python_createUniformBlockGrid
//{
// public:
//   static std::shared_ptr< StructuredBlockForest > create(py::args args, py::kwargs kw)
//   {
//      if (py::len(std::move(args)) > 0) { throw py::value_error("This function takes only keyword arguments"); }
//
//      for (auto item : kw)
//      {
//         std::string key = py::str(item.first);
//         if (key != "cells" && key != "cellsPerBlock" && key != "blocks" && key != "periodic" && key != "dx" &&
...
//            "Parameter 'cellsPerBlock' has to be tuple of length 3, indicating cells in x,y,z direction");
//      }
...
//            createUniformBlockGridFromConfig(cfg->getGlobalBlock(), nullptr, keepGlobalBlockInformation);
//         return blocks;
...
//class createStructuredBlockForest
//{
// public:
//   static shared_ptr< StructuredBlockForest >
//      create(Vector3< uint_t > blocks, Vector3< uint_t > cellsPerBlock, Vector3< bool > periodic,
//             py::object blockExclusionCallback, py::object workloadMemoryCallback, py::object refinementCallback,
//             const real_t dx = 1.0, memory_t processMemoryLimit = std::numeric_limits< memory_t >::max(),
...
//
//      auto blockExclusionFunc = [&blockExclusionCallback](std::vector< walberla::uint8_t >& excludeBlock,
//                                                          const SetupBlockForest::RootBlockAABB& aabb) -> void {
//         for (uint_t i = 0; i != excludeBlock.size(); ++i)
//         {
//            AABB bb              = aabb(i);
//            auto pythonReturnVal = blockExclusionCallback(bb);
//            if (py::isinstance< bool >(pythonReturnVal))
//            { throw py::value_error("blockExclusionCallback has to return a boolean"); }
//
//            bool returnVal = bool(pythonReturnVal);
//            if (returnVal) excludeBlock[i] = 1;
//         }
//      };
//
...
//            if (returnVal) block->setMarker(true);
//         }
//      };
//
...
//                          uint_c(MPIManager::instance()->numProcesses()), real_t(0), processMemoryLimit);
//
...
void exportBlockForest(py::module_& m)
{
   ...
   py::class_< math::AABB >(m, "AABB")
      .def_property_readonly("max",
                             [](const math::AABB& self) {
                               return std::array< real_t, 3 >{ self.xMax(), self.yMax(), self.zMax() };
                             })
      .def_property_readonly("min",
                             [](const math::AABB& self) {
                               return std::array< real_t, 3 >{ self.xMin(), self.yMin(), self.zMin() };
                             })
      .def_property_readonly("center",
                             [](const math::AABB& self) {
                               return std::array< real_t, 3 >{ self.xMin() + real_c(0.5) * self.xSize(),
                                                               self.yMin() + real_c(0.5) * self.ySize(),
                                                               self.zMin() + real_c(0.5) * self.zSize() };
                             })
      .def_property_readonly("size", [](const math::AABB& self) {
        return std::array< real_t, 3 >{ self.xSize(), self.ySize(), self.zSize() };
      });
   ...
   m.def("createCustomBlockGrid",
         [](Vector3< uint_t > blocks, Vector3< uint_t > cellsPerBlock, Vector3< bool > periodic,
            std::function<bool(AABB)> blockExclusionCallback, std::function<void(SetupBlock*)> workloadMemoryCallback, std::function<bool(SetupBlock*)> refinementCallback,
            const real_t dx = 1.0, memory_t processMemoryLimit = std::numeric_limits< memory_t >::max(),
            const bool keepGlobalBlockInformation = false)
         {
           using namespace blockforest;
           ...
           auto blockExclusionFunc = [&blockExclusionCallback](std::vector< walberla::uint8_t >& excludeBlock,
             ...
             {
                AABB bb              = aabb(i);
                bool pythonReturnVal = blockExclusionCallback(bb);
         ...
         "blocks"_a, "cellsPerBlock"_a, "dx"_a = real_t(1), "periodic"_a = std::array< bool, 3 >{ false, false, false },
         "processMemoryLimit"_a = std::numeric_limits< memory_t >::max(), "keepGlobalBlockInformation"_a = false,
         "blockExclusionCallback"_a, "workloadMemoryCallback"_a, "refinementCallback"_a
         );

//   py::class_< createStructuredBlockForest >(m, "createCustomBlockGrid")
//      .def_static("create", &createStructuredBlockForest::create, py::arg("blocks"), py::arg("cellsPerBlock"),
//                  py::arg("periodic"), py::arg("blockExclusionCallback"), py::arg("workloadMemoryCallback"),
//                  py::arg("refinementCallback"), py::arg("dx") = 1.0,
