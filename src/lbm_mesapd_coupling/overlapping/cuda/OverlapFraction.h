...
struct OverlapFractionFunctor
{
                   ...
                   const BlockDataID& particleAndVolumeFractionFieldID)
   {
      ...
      auto cudaField = blockIt.getData< walberla::cuda::GPUField< ParticleAndVolumeFractionAoS_T > >(particleAndVolumeFractionFieldID);

      auto myKernel = walberla::cuda::make_kernel(&particleAndVolumeFractionMappingKernelAoS);
      myKernel.addFieldIndexingParam(walberla::cuda::FieldIndexing< ParticleAndVolumeFractionAoS_T >::xyz(*cudaField)); // FieldAccessor
