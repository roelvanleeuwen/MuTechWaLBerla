...
void simpleCreateAndIterate(field::Layout layout)
{
   ...
   Field< uint_t > field(xs, ys, zs, fs, layout);
...
void alignmentTest()
{
   ...
   Field< short > field(xs, ys, zs, fs, field::fzyx, alloc);
   for (cell_idx_t f = 0; f < cell_idx_c(field.fSize()); ++f)
      for (cell_idx_t z = 0; z < cell_idx_t(field.zSize()); ++z)
         for (cell_idx_t y = 0; y < cell_idx_t(field.ySize()); ++y)
         {
...
void ghostLayerFieldAlignmentTest()
{
   ...
   shared_ptr< Allocator > alloc = make_shared< Allocator >();
   GhostLayerField< double > field(xs, ys, zs, fs, gl, field::fzyx, alloc);
   for (cell_idx_t f = 0; f < cell_idx_c(field.fSize()); ++f)
      for (cell_idx_t z = 0; z < cell_idx_t(field.zSize()); ++z)
         for (cell_idx_t y = 0; y < cell_idx_t(field.ySize()); ++y)
         {
...
void blockedIterTest(field::Layout layout)
{
   ...
   Field< uint_t > field(xs, ys, zs, fs, layout);
   for (Field< uint_t >::iterator i = field.begin(); i != field.end(); ++i)
      field(i.x(), i.y(), i.z(), i.f()) = uint_c(i.x() + i.y() + i.z() + i.f());

   int counter = 0;
   for (Field< uint_t >::iterator i = field.beginSlice(5, 5, 5, 1, 7, 7, 7, 3); i != field.end(); ++i)
   {
      // cout << counter << "\t(" <<i.f() << "," << i.z() << "," << i.y() << "," << i.x() <<") Value ="<< *i << endl;
      // cout << i << endl;
      counter++;
...
void ghostLayerFieldCreateAndIterate(field::Layout layout)
{
   ...
   GhostLayerField< cell_idx_t > field(xs, ys, zs, fs, gl, layout);
...
void neighborTest(field::Layout layout)
{
   ...
   GhostLayerField< uint_t > myField(xs, ys, zs, fs, gl, 0, layout);
...
void ghostlayerIterators(field::Layout layout)
{
   ...
   GhostLayerField< std::string > field(1, 1, 1, 1, 1, layout);
...
{
   const uint_t gl = 3;
   GhostLayerField< int > field(3, 4, 5, 3, gl, layout, make_shared< field::StdFieldAlloc< int > >());
...
{
   GhostLayerField< int > field1(2, 2, 2, 1, 1, 1, layout);
   GhostLayerField< int > field2(2, 2, 2, 1, 1, 2, layout);
...
void sliceTest(field::Layout layout)
{
   ...
   Field< int > field(3, 3, 3, fSize, ELSE_VAL, layout);
   ...
   {
      ++counter;
      if (i.f() == 0 && sliceInterval.contains(i.cell())) { WALBERLA_CHECK_EQUAL(*i, SLICE_VAL) }
      else
      {
   ...
   for (cell_idx_t z = 0; z < cell_idx_c(sliceInterval.zSize()); ++z)
      for (cell_idx_t y = 0; y < cell_idx_c(sliceInterval.ySize()); ++y)
         for (cell_idx_t x = 0; x < cell_idx_c(sliceInterval.xSize()); ++x)
            ...
            {
               ++counter;
               if (f == 0) { WALBERLA_CHECK_EQUAL(sliced->get(x, y, z, f), SLICE_VAL) }
               else
               {
   ...
   {
      ++counter;
      if (i.f() == 0) { WALBERLA_CHECK_EQUAL(*i, SLICE_VAL) }
      else
      {
         WALBERLA_CHECK_EQUAL(*i, ELSE_VAL)
      }
...
void printerTest()
{
   GhostLayerField< double > field(5, 5, 5, 1, 2, 42.424242);
...
void reverseIteratorTest(field::Layout layout)
{
   ...
   GhostLayerField< cell_idx_t > field(xs, ys, zs, fs, gl, layout);
   const GhostLayerField< cell_idx_t >& constField = field;
...
void isIteratorConsecutiveTest(field::Layout layout)
{
   ...
   GhostLayerField< int > field(xs, ys, zs, fs, gl, 0, layout, make_shared< field::StdFieldAlloc< int > >());
...
void swapableCompareTest()
{
   ...
   fieldSet.insert(new MyField(2, 4, 1, 1, field::zyxf));
   fieldSet.insert(new MyField(1, 2, 5, 2, field::zyxf));
   fieldSet.insert(new MyField(1, 2, 2, 3, field::zyxf));
   fieldSet.insert(new MyField(1, 2, 3, field::zyxf));

   fieldSet.insert(new MyField(2, 4, 1, 1, field::fzyx));
   fieldSet.insert(new MyField(1, 2, 5, 27, field::fzyx));
   fieldSet.insert(new MyField(1, 2, 2, 4, field::fzyx));
   fieldSet.insert(new MyField(1, 2, 3, 1, field::fzyx));

   for (auto i = fieldSet.begin(); i != fieldSet.end(); ++i)
   {
      std::cout << "( " << (*i)->xSize() << "," << (*i)->ySize() << "," << (*i)->zSize() << "," << (*i)->fSize() << ","
                << (*i)->layout() << ")" << std::endl;
   }
...
real_t functionTakingAConstIterator(const GhostLayerField< real_t >::const_base_iterator& it) { return *it; }

void iteratorToConstConversionTest()
{
   GhostLayerField< real_t > glField(1, 1, 1, 1, 1);
...
void sizeTest()
{
   ...
   GhostLayerField< cell_idx_t > field(xs, ys, zs, fs, gl, field::fzyx);
...
void fieldPointerTest()
{
   ...
   Field< real_t >::Ptr fieldPtr(field, 0, 0, 0);
...
void flattenTest()
{
   Field< Vector3< uint_t > > field(2, 2, 1, fSize);
   ...
   Field< uint_t > cmp(2, 2, 1, 3 * fSize);
...
{
   GhostLayerField< Vector3< uint_t > > field(2, 2, 1, fSize, 1);

   for (cell_idx_t x = -cell_idx_c(field.nrOfGhostLayers()); x < cell_idx_c(field.xSize() + field.nrOfGhostLayers());
        ++x)
      for (cell_idx_t y = -cell_idx_c(field.nrOfGhostLayers()); y < cell_idx_c(field.ySize() + field.nrOfGhostLayers());
           ++y)
         for (cell_idx_t z = -cell_idx_c(field.nrOfGhostLayers());
              z < cell_idx_c(field.zSize() + field.nrOfGhostLayers()); ++z)
            for (cell_idx_t f = 0; f < cell_idx_c(field.fSize()); ++f)
               for (uint_t g = 0; g < 3; ++g)
               {
                  uint_t val           = uint_t(&(field(x, y, z, f)[g]));
   ...
   GhostLayerField< uint_t > cmp(2, 2, 1, 3 * fSize, 1);
   WALBERLA_CHECK_EQUAL(cmp.xSize(), flattened->xSize())
   WALBERLA_CHECK_EQUAL(cmp.ySize(), flattened->ySize())
   WALBERLA_CHECK_EQUAL(cmp.zSize(), flattened->zSize())
   WALBERLA_CHECK_EQUAL(cmp.fSize(), flattened->fSize())
